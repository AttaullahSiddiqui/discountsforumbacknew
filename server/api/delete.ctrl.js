...
function deleteStore(req, res) {
  Store.deleteOne({ _id: req.body._id }, function (err, deletedNode) {
    if (err) res.json(resHandler.respondError(err[0], err[1] || -1));
      ...
      res.json(
        resHandler.respondSuccess(deletedNode, "Store deleted successfully", 2)
      );
    }
  });
}
function deleteBanner(req, res) {
  Banner.deleteOne({ _id: req.body._id }, function (err, deletedNode) {
    if (err) res.json(resHandler.respondError(err[0], err[1] || -1));
      ...
      res.json(
        resHandler.respondSuccess(deletedNode, "Banner deleted successfully", 2)
      );
    }
  });
}
  ...
  BlogItem.deleteOne({ _id: req.body._id }, function (err, deletedNode) {
    if (err) res.json(resHandler.respondError(err[0], err[1] || -1));
      ...
      res.json(
        resHandler.respondSuccess(
          deletedNode,
          "Blog Item deleted successfully",
          2
        )
      );
    }
...
function deleteCoupon(req, res) {
  Coupon.deleteOne({ _id: req.body._id }, function (err, deletedNode) {
    if (err) res.json(resHandler.respondError(err[0], err[1] || -1));
      ...
      res.json(
        resHandler.respondSuccess(deletedNode, "Coupon deleted successfully", 2)
      );
    }
  });
}
function deleteCouponCallback(couponDetail) {
  Coupon.updateMany({ storeId: couponDetail.storeId }, { $inc: { sortNo: -1 } })
    .where("sortNo")
    .gt(couponDetail.sortNo)
...
function deleteBlog(req, res) {
  Blog.deleteOne({ _id: req.body._id }, function (err, deletedNode) {
    if (err) res.json(resHandler.respondError(err[0], err[1] || -1));
      ...
      res.json(
        resHandler.respondSuccess(deletedNode, "Blog deleted successfully", 2)
      );
  });
}
function deleteUser(req, res) {
  User.deleteOne({ _id: req.body._id }, function (err, deletedNode) {
    if (err) res.json(resHandler.respondError(err[0], err[1] || -1));
      ...
      res.json(
        resHandler.respondSuccess(deletedNode, "User deleted successfully", 2)
      );
  });
}
