...
function fetchBlogsOnlyId(req, res) {
    Blog.find({}, 'title blogURL', { "sort": ['title'] }, function (err, blogs) {
        if (err) res.json(resHandler.respondError(err[0], err[1] || -1));
        else if (!blogs) res.json(resHandler.respondError("No Blogs at the moment", -3));
...
function fetchStoreByIdDuplicate(req, res) {
    Store.findOne({ _id: req.query._id }, function (err, store) {
        ...
        else res.json(resHandler.respondError("Unable to fetch selected store", -3));
    })
}
...
function fetchCouponsForSorting(req, res) {
    Coupon.
        find({ storeId: req.query._id }, 'sortNo offerBox').
        skip(Number(req.query.skipNo)).
        limit(Number(req.query.limitNo)).
        sort({ sortNo: 1 }).
        exec(function (err, coupons) {
    ...
    Banner.
        ...
        exec(function (err, banners) {
            if (err) res.json(resHandler.respondError(err[0], err[1] || -1));
                ...
                if (banners.length) res.json(resHandler.respondSuccess(banners, "Banners fetched successfully", 2));
                else res.json(resHandler.respondError("No Banner found in this Store", -3));
            }
        });
}
function fetchPostImagesById(req, res) {
    postImg.find({ storeId: req.query._id }).
        sort({ CreatedAt: -1 }).
        exec(function (err, postImgs) {
            if (err) res.json(resHandler.respondError(err[0], err[1] || -1));
            else {
                if (postImgs.length) res.json(resHandler.respondSuccess(postImgs, "Post Images fetched successfully", 2));
                else res.json(resHandler.respondError("No Post Image found in this Store", -3));
            }
        });
}
    ...
    Product.find({ storeId: req.query._id }).
        ...
        exec(function (err, products) {
            if (err) res.json(resHandler.respondError(err[0], err[1] || -1));
                ...
                if (products.length) res.json(resHandler.respondSuccess(products, "Products fetched successfully", 2));
                else res.json(resHandler.respondError("No Product found in this Store", -3));
            }
        });
}
    ...
    BlogItem.find({ blogId: req.query._id }).
        ...
        exec(function (err, items) {
            if (err) res.json(resHandler.respondError(err[0], err[1] || -1));
                ...
                if (items.length) res.json(resHandler.respondSuccess(items, "Blog Items fetched successfully", 2));
                else res.json(resHandler.respondError("No Item found in this Blog", -3));
            }
        });
}
...
function fetchSettingsData(req, res) {
    Settings.findOne({ _id: "60efd3f2fba2a17903aa5643" }, function (err, settings) {
        if (err) res.json(resHandler.respondError(err[0], err[1] || -1));
        else if (!settings) res.json(resHandler.respondError("Unable to fetch Settings at the moment", -3));
        else res.json(resHandler.respondSuccess(settings, "Settings Data fetched successfully", 2));
    })
}
function fetchEmails(req, res) {
    userEmails.find({}, function (err, emails) {
        if (err) res.json(resHandler.respondError(err[0], err[1] || -1));
        else if (!emails) res.json(resHandler.respondError("Unable to fetch Emails at the moment", -3));
        else res.json(resHandler.respondSuccess(emails, "Emails fetched successfully", 2));
    })
}