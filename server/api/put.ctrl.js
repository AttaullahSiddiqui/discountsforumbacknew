...
function editStore(req, res) {
    ...
    Store.findByIdAndUpdate(req.body._id, req.body, { new: true }, function (err, updatedNode) {
        if (err) res.json(resHandler.respondError(err[0], err[1] || -1));
            ...
            Coupon.updateMany({ storeId: req.body._id }, { trackingLink: req.body.trackUrl }, function (error, result) {
...
function updateBanner(req, res) {
    ...
    Banner.findByIdAndUpdate(req.body._id, req.body, { new: true }, function (err, updatedNode) {
        if (err) res.json(resHandler.respondError(err[0], err[1] || -1));
        else if (!updatedNode) res.json(resHandler.respondError("Wrong format provided", -3));
        else res.json(resHandler.respondSuccess(updatedNode, "Banner updated successfully", 2));
    })
}
function updatePostImage(req, res) {
    delete req.body.trigger;
    postImg.findByIdAndUpdate(req.body._id, req.body, { new: true }, function (err, updatedNode) {
        if (err) res.json(resHandler.respondError(err[0], err[1] || -1));
        else if (!updatedNode) res.json(resHandler.respondError("Wrong format provided", -3));
        else res.json(resHandler.respondSuccess(updatedNode, "Post Image updated successfully", 2));
    })
}
function updateProduct(req, res) {
    Product.findByIdAndUpdate(req.body._id, req.body, function (err, updatedNode) {
        if (err) res.json(resHandler.respondError(err[0], err[1] || -1));
        else if (!updatedNode) res.json(resHandler.respondError("Wrong format provided", -3));
        else res.json(resHandler.respondSuccess(updatedNode, "Product updated successfully", 2));
    })
}
function updateBlogItem(req, res) {
    BlogItem.findByIdAndUpdate(req.body._id, req.body, function (err, updatedNode) {
        if (err) res.json(resHandler.respondError(err[0], err[1] || -1));
        else if (!updatedNode) res.json(resHandler.respondError("Wrong format provided", -3));
        else res.json(resHandler.respondSuccess(updatedNode, "Blog Item updated successfully", 2));
    })
}
        ...
        exec(function (err, coupons) {
            if (err) res.json(resHandler.respondError(err[0], err[1] || -1));
                ...
                if (coupons) {
                    ...
                    setTimeout(function () {
                        res.json(resHandler.respondSuccess(coupons, "Date for All Coupons updated successfully", 2))
                    }, 1000);
                }
                else res.json(resHandler.respondError("Unable to change date of All Coupons", -3));
            }
        });
}
function changeDateCallBack(coupons, newDate) {
    coupons.forEach(elm => {
        Coupon.updateOne({ _id: elm._id }, { expDate: newDate }, function (err, data) {
...
function updateSettings(req, res) {
    ...
    // })
    Settings.findByIdAndUpdate("60efd3f2fba2a17903aa5643", req.body, function (err, data) {
        if (err) res.json(resHandler.respondError(err[0], err[1] || -1));
        else if (!data) res.json(resHandler.respondError("Wrong format provided", -3));
        else res.json(resHandler.respondSuccess(data, "Settings updated successfully", 2));
    })
}
