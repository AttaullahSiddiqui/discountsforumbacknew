        ...
        if (err) res.json(resHandler.respondError(err[0], err[1] || -1));
            ...
            if (req.body.userPass == fetchedUser.userPass) {
                jwt.generateToken({ userID: fetchedUser._id }, function (jwtErr, jwtSuccess) {
                    ...
                    res.json(resHandler.respondSuccess(loggedUser, "User login successfully", 1));
                });
...
function addBanner(req, res) {
    ...
    newBanner.save().then(function (result) {
        res.json(resHandler.respondSuccess(result, "Banner added successfully", 2));
    }, function (err) {
        var error = errHandler.handle(err);
        res.json(resHandler.respondError(error[0], (error[1] || -1)));
    })
...
function addPostImage(req, res) {
    ...
    newPost.save().then(function (result) {
        res.json(resHandler.respondSuccess(result, "Post Image added successfully", 2));
    }, function (err) {
        var error = errHandler.handle(err);
        res.json(resHandler.respondError(error[0], (error[1] || -1)));
    })
...
function addBlogItem(req, res) {
    ...
    newBlogItem.save().then(function (result) {
        res.json(resHandler.respondSuccess(result, "Blog Item added successfully", 2));
    }, function (err) {
        var error = errHandler.handle(err);
        res.json(resHandler.respondError(error[0], (error[1] || -1)));
    })
...
function addCoupon(req, res) {
    Coupon.find({ storeId: req.body.storeId }).sort({ sortNo: -1 }).limit(1).then((coupon) => {
        if (coupon[0]) {
...
function addCouponCallback(req, res) {
    ...
    newCoupon.save().then(function (result) {
        Store.updateOne({ _id: req.body.storeId }, { CreatedAt: Date.now() }, function (err, data) {
